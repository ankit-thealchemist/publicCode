...
{   
    ...
    /**
     * @brief Reading the internal mac address of the esp32
     * 
     */
    ESP_ERROR_CHECK_WITHOUT_ABORT(esp_read_mac(mac_addr,ESP_MAC_ETH));

    ESP_LOGI(TAG,"mac address is: %02x:%02x:%02x:%02x:%02x:%02x",mac_addr[0],mac_addr[1],mac_addr[2],mac_addr[3],mac_addr[4],mac_addr[5]);

    /**
     * @brief setting the mac address of the enc28j60
     * 
     */
