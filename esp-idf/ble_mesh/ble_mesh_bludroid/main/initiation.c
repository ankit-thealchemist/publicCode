...
static esp_ble_mesh_elem_t elements[] = {
    ...
    ESP_BLE_MESH_ELEMENT(1, extend_model_0, ESP_BLE_MESH_MODEL_NONE),
    ESP_BLE_MESH_ELEMENT(2, extend_model_1, ESP_BLE_MESH_MODEL_NONE),
};
...
const uint8_t static_oob_val[] = {0x01};
...
void mesh_intitializaton(){
    ...
    if (ret) {
        ESP_LOGE(bleTag, "Initializing mesh failed (err %s)", ret);
        return ;
    }
