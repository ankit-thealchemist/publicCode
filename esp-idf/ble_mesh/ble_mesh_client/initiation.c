...
#define bleTag "mesh_init"

//const uint8_t static_oob_val[] = {0x6,0x1};
...
ESP_BLE_MESH_MODEL_PUB_DEFINE(sensor_cli_pub, 2 + 1, ROLE_NODE); // defining the publication for the client publication 
...
static esp_ble_mesh_model_t root_models[] = {   // this is the root model or the model for the primary element 
    ...
    ESP_BLE_MESH_MODEL_GEN_ONOFF_CLI(&sensor_cli_pub, &sensor_client),
};
...
//    .static_val_len = sizeof(static_oob_val),
    .uuid = dev_uuid, 
 //   .oob_info= ESP_BLE_MESH_PROV_OOB_OTHER,
};
